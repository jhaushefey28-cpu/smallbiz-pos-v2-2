...
function App(){
  ...
  const externalSidebarOpen={
    ...
    "growth":async()=>{requestSidebarModuleOpen("smallbiz:open-growth-center");await import("./growth-center.jsx");window.dispatchEvent(new CustomEvent("smallbiz:open-growth-center"));},
  ...
  if(!session)return <div className="auth">
    {authMode==="login"&&<form className="login-card" onSubmit={login}><div className="login-logo">🛒</div><h1>SmallBiz POS</h1><p>Sign in to your business account</p><input type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} required/><div style={{position:"relative",width:"100%"}}><input type={showPassword?"text":"password"} placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} required style={{width:"100%",boxSizing:"border-box",paddingRight:44}}/><button type="button" onClick={()=>setShowPassword(v=>!v)} aria-label={showPassword?"Hide password":"Show password"} style={{position:"absolute",right:4,top:"50%",transform:"translateY(-50%)",width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",border:0,background:"transparent",cursor:"pointer",fontSize:16,padding:0,lineHeight:1,zIndex:2}}>{showPassword?"🙈":"👁️"}</button></div><button className="primary">Login</button><button type="button" onClick={()=>{setErr("");setResetSent(false);setAuthMode("forgot")}} style={{background:"none",border:0,color:"#1769e0",cursor:"pointer",fontSize:13,marginTop:4}}>Forgot password?</button>{err&&<p className="error">{err}</p>}</form>}
