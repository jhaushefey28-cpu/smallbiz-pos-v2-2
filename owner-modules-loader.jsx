// SMALLBIZ_OWNER_MODULES_LOADER_V33_STABLE_CANONICAL
// Canonical owner menu entries are created once. Existing React entries are never
// intercepted in capture phase. Module loading must not recreate sidebar items.
...
function ensureCanonicalButton(item){
  ...
  const marked=Array.from(root.querySelectorAll(`[data-smallbiz-owner-canonical='${item.key}']`));
  const matches=matchingButtons(root,item);
  // Prefer the first existing React button. Do not replace or intercept it.
  const existing=matches.find(el=>!el.dataset.smallbizOwnerCreated)||null;
  if(existing){
    existing.dataset.smallbizOwnerCanonical=item.key;
    removeDuplicates(root,item,existing);
    return existing;
  }
  const canonical=marked[0]||matches[0]||null;
  if(canonical){
    removeDuplicates(root,item,canonical);
    return bindCreatedButton(item,canonical);
  }
  const button=document.createElement("button");
  button.className="nav-item";
  button.innerHTML='<span aria-hidden="true">'+item.icon+'</span><b>'+item.label+'</b>';
  root.appendChild(button);
  return bindCreatedButton(item,button);
}
...
async function loadOwnerModule(item,button){
  ...
  try{
    ...
    // Cleanup only after the module has finished initializing. Never append here.
    const root=nav();
    if(root){
      const canonical=Array.from(root.querySelectorAll(`[data-smallbiz-owner-canonical='${item.key}']`))[0]||matchingButtons(root,item)[0];
      if(canonical)removeDuplicates(root,item,canonical);
    }
  }catch(error){
...
function scheduleReconcile(){
  ...
  reconcileTimer=setTimeout(reconcileOwnerMenu,120);
}
...
function installSidebarObserver(){
  ...
  window[GLOBAL_KEY]={...state,observerInstalled:true,observer};
  scheduleReconcile();
}
