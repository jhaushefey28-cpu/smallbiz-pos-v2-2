// SMALLBIZ_OWNER_MODULES_LOADER_V31_REACT_SAFE_SINGLE_OWNER
// Canonical owner sidebar reconciliation: restores missing owner menus after React renders,
...
function removeDuplicateOwnerButtons(root,item,keep){matchingButtons(root,item).forEach(el=>{if(el!==keep&&el.dataset.smallbizOwnerCreated==="v31")el.remove()});Array.from(root.querySelectorAll(`[data-smallbiz-owner-canonical='${item.key}']`)).forEach(el=>{if(el!==keep&&el.dataset.smallbizOwnerCreated==="v31")el.remove()})}
function bindCanonicalButton(item,button,created=false){if(!button)return null;button.type="button";button.setAttribute("aria-label",item.label);button.style.setProperty("pointer-events","auto","important");button.style.setProperty("touch-action","manipulation","important");if(created)button.dataset.smallbizOwnerCreated="v31";if(button.dataset.smallbizOwnerBound!=="v31"){button.addEventListener("click",event=>{if(event.defaultPrevented)return;loadOwnerModule(item,button)},{passive:false});button.dataset.smallbizOwnerBound="v31"}button.dataset.smallbizOwnerCanonical=item.key;return button}
function ensureCanonicalButton(item){const root=nav();if(!root||!hasPermission(item.permission))return null;let canonical=root.querySelector(`[data-smallbiz-owner-canonical='${item.key}']`);if(!canonical)canonical=matchingButtons(root,item)[0]||null;if(canonical){removeDuplicateOwnerButtons(root,item,canonical);return bindCanonicalButton(item,canonical,false)}const button=document.createElement("button");button.className="nav-item";button.dataset.smallbizOwnerCanonical=item.key;button.innerHTML='<span aria-hidden="true">'+item.icon+'</span><b>'+item.label+'</b>';root.appendChild(button);return bindCanonicalButton(item,button,true)}
...
function scheduleReconcile(){clearTimeout(reconcileTimer);reconcileTimer=setTimeout(()=>reconcileOwnerMenu(),60)}
function installSidebarObserver(){const state=window[GLOBAL_KEY]||{};if(state.observerInstalled)return;if(!document.body)return;const observer=new MutationObserver(mutations=>{if(mutations.some(m=>m.type==="childList"&&m.addedNodes.length))scheduleReconcile()});observer.observe(document.body,{childList:true,subtree:true});window[GLOBAL_KEY]={...state,observerInstalled:true,observer};scheduleReconcile()}
