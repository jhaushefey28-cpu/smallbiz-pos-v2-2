(() => {
  ...
  function up(e){if(!drag||e.pointerId!==drag.id||!bubble)return;const moved=drag.moved;if(moved){const r=bubble.getBoundingClientRect(),p=clamp(r.left,r.top);bubble.style.left=p.x+'px';bubble.style.top=p.y+'px';bubble.style.right='auto';bubble.style.bottom='auto';localStorage.setItem(POS_KEY,JSON.stringify(p));bubble.dataset.dragged='1';setTimeout(()=>{if(bubble)bubble.dataset.dragged='0'},0)}drag=null}
