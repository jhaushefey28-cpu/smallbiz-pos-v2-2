// SMALLBIZ_TEAM_MANAGEMENT_V3
// Standalone tenant-scoped Team UI. Opens from the lazy sidebar without changing mobile layout.
...
async function getContext(){
  ...
  if(pe)throw new Error(pe.message);if(!profile?.business_id)throw new Error("Business profile not found.");
  ...
  const allowed=Boolean(tenantAdmin?.user_id===uid)||["owner","admin","super_admin"].includes(role);
  if(!allowed)throw new Error("Tenant Super Admin/Owner access is required for Team Management.");
  return {profile,uid};
}
...
async function openPermissions(member){
  ...
  o.querySelector("#team-perm-save").onclick=async()=>{
    ...
    try{
      if(!ctx.profile?.business_id)throw new Error("Business context missing.");
      const selected=[...o.querySelectorAll("[data-permission-id]:checked")].map(x=>x.getAttribute("data-permission-id"));
      const {error:de}=await sb.from("user_permissions").delete().eq("user_id",member.user_id);if(de)throw new Error(de.message);
      if(selected.length){const {error:ie}=await sb.from("user_permissions").insert(selected.map(permission_id=>({user_id:member.user_id,permission_id,allowed:true})));if(ie)throw new Error(ie.message);}
      window.dispatchEvent(new CustomEvent("smallbiz:user-permissions-updated",{detail:{userId:member.user_id}}));
...
async function openCreate(){
  const ctx=await getContext();const o=cardShell();
  o.innerHTML=`<div style="width:min(600px,96vw);background:#fff;border-radius:18px;padding:20px;color:#172033;box-shadow:0 24px 80px rgba(0,0,0,.28);font-family:system-ui"><div style="display:flex;justify-content:space-between"><div><div style="font-size:12px;color:#64748b;font-weight:800">TEAM MANAGEMENT</div><h2 style="margin:4px 0">👤 Add Team Account</h2></div><button id="team-create-close" type="button" style="border:0;background:#f1f5f9;border-radius:10px;width:40px;height:40px">✕</button></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:16px"><label>Full Name<input id="tm-name" style="width:100%;box-sizing:border-box;padding:10px;margin-top:5px" placeholder="Juan Dela Cruz"></label><label>Email<input id="tm-email" type="email" style="width:100%;box-sizing:border-box;padding:10px;margin-top:5px" placeholder="juan@example.com"></label><label>Role<select id="tm-role" style="width:100%;box-sizing:border-box;padding:10px;margin-top:5px"><option value="cashier">Cashier</option><option value="staff">Staff</option><option value="manager">Manager</option><option value="admin">Admin</option></select></label><label>Temporary Password<input id="tm-password" type="password" style="width:100%;box-sizing:border-box;padding:10px;margin-top:5px" placeholder="At least 8 characters"></label></div><div id="tm-create-msg" style="font-size:12px;margin-top:12px"></div><div style="display:flex;justify-content:flex-end;gap:8px;margin-top:14px"><button id="tm-create-cancel" type="button">Cancel</button><button id="tm-create-save" type="button" class="primary">Create Account</button></div></div>`;
  o.querySelector("#team-create-close").onclick=closeTeam;o.querySelector("#tm-create-cancel").onclick=closeTeam;
  o.querySelector("#tm-create-save").onclick=async()=>{const btn=o.querySelector("#tm-create-save"),msg=o.querySelector("#tm-create-msg");btn.disabled=true;msg.textContent="Creating account...";try{const {data,error}=await sb.functions.invoke("team-admin-user",{body:{action:"create",full_name:o.querySelector("#tm-name").value.trim(),email:o.querySelector("#tm-email").value.trim().toLowerCase(),role:o.querySelector("#tm-role").value,password:o.querySelector("#tm-password").value,business_id:ctx.profile.business_id}});if(error)throw new Error(error.message);if(data?.error)throw new Error(data.error);msg.style.color="#166534";msg.textContent=`Account created: ${data?.email||o.querySelector("#tm-email").value.trim()}`;setTimeout(closeTeam,800);}catch(e){msg.style.color="#991b1b";msg.textContent=e.message||"Unable to create account.";btn.disabled=false;}};
}

async function openTeam(){
  try{
    const ctx=await getContext();
    const members=await loadMembers(ctx.profile.business_id);
    const o=cardShell();
    o.innerHTML=`<div id="team-management-modal" style="width:min(1000px,96vw);max-height:92dvh;overflow:auto;background:#fff;border-radius:18px;padding:20px;color:#172033;box-shadow:0 24px 80px rgba(0,0,0,.28);font-family:system-ui"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start"><div><div style="font-size:12px;color:#64748b;font-weight:800">SMALLBIZ POS</div><h2 style="margin:4px 0">👥 Team Management</h2><p style="margin:0;color:#64748b">Manage users and tenant-specific sidebar permissions.</p></div><button id="team-close" type="button" style="border:0;background:#f1f5f9;border-radius:10px;width:40px;height:40px">✕</button></div><div style="display:flex;justify-content:flex-end;margin:14px 0"><button id="tm-add-cashier" type="button" class="primary">➕ Add Team Account</button></div><div class="table-wrapper"><table class="tm-table" style="width:100%"><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Joined</th><th>Action</th></tr></thead><tbody>${members.length?members.map(m=>`<tr><td><b>${esc(m.full_name||m.name||"User")}</b></td><td>${esc(m.email||"")}</td><td>${esc(m.role||"cashier")}</td><td>${m.active===false?"Inactive":"Active"}</td><td>${m.created_at?new Date(m.created_at).toLocaleDateString("en-PH"):"-"}</td><td><button type="button" data-team-active="${esc(m.user_id||m.id||"")}" data-team-email="${esc(m.email||"")}" data-team-name="${esc(m.full_name||m.name||"User")}" data-team-role="${esc(m.role||"cashier")}">Manage Access</button></td></tr>`).join(""):"<tr><td colspan=\"6\" style=\"text-align:center;padding:30px\">No team members found.</td></tr>"}</tbody></table></div><div id="team-manage-msg" style="margin-top:12px;font-size:12px;color:#64748b">Tenant: ${esc(ctx.profile.business_id)}</div></div>`;
    ...
    console.error("[SmallBiz] Team Management failed:",e);
    alert(e.message||"Unable to open Team Management.");
  }
}

window.__smallbizOpenTeam=openTeam;
window.__SMALLBIZ_TEAM_MANAGEMENT__="v3";
