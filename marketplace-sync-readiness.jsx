...
function Panel({profile,onClose}){
 const [rows,setRows]=useState([]),[maps,setMaps]=useState([]),[orders,setOrders]=useState([]),[loading,setLoading]=useState(true),[error,setError]=useState(""),[msg,setMsg]=useState("");
 ...
 <div className="msr-note"><b>Next connector step:</b> configure each marketplace's official API credentials and server-side adapter. We do not fake order synchronization from the browser. Once authorized, imported orders will land in the existing <code>external_orders</code> / <code>external_order_items</code> pipeline and become available to accounting reports.</div>
