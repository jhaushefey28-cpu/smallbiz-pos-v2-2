...
function findExistingButton(item){
 const root=nav();
 if(!root)return null;
 const targets=new Set(labelsFor(item));
 return Array.from(root.querySelectorAll("button,a,[role='button']")).find(el=>targets.has(normalizedLabel(textOf(el))))||null;
}

function bindButton(item,button){
 if(!button)return null;
 button.type="button";
 button.setAttribute("aria-label",item.label);
 button.style.setProperty("pointer-events","auto","important");
 button.style.setProperty("touch-action","pan-y","important");
 button.style.setProperty("position","relative","important");
 button.style.setProperty("z-index","3","important");
 if(button.dataset.smallbizOwnerBound!=="v26"){
  button.onclick=event=>{
   event.preventDefault();
   event.stopPropagation();
   loadOwnerModule(item,button);
  };
  button.dataset.smallbizOwnerBound="v26";
 }
 return button;
}
...
function ensureButton(item){
 const root=nav();
 if(!root)return null;
 const existing=findExistingButton(item);
...
function reconcileOwnerMenu(){
 ...
 OWNER_MENU.forEach(item=>{
  if(hasPermission(item.permission))ensureButton(item);
 });
 return true;
}

function scheduleReconcile(){
 [0,250,750].forEach(delay=>setTimeout(reconcileOwnerMenu,delay));
}
...
console.log("Applied SMALLBIZ_OWNER_SIDEBAR_LAG_FIX_V26_SAFE: preserved all existing sidebar items and removed destructive sidebar reconciliation.");
